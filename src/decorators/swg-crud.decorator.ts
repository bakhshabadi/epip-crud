    ...
    return [
      { $ref: getSchemaPath(IResponse) },
      {
        properties: {
          result: {
            $ref: getSchemaPath(
              model.name !== 'Object' ? model.name : AnyResponse.name,
            ),
          },
        },
      },
...
export const ApiPostAll = <TModel extends Type<any>>(
  model: TModel,
  path?: string,
) => {
  if (!path) {
...
export const ApiPutAll = <TModel extends Type<any>>(
  model: TModel,
  path?: string,
) => {
  if (!path) {
...
export const ApiPatchAll = <TModel extends Type<any>>(
  model: TModel,
  path?: string,
) => {
  if (!path) {
